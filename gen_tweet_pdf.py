...
读取仓库内 tweet_data 数据，固定文件名，每次生成直接覆盖旧PDF
输出目录：pdf_output/
固定文件名：推文合集.pdf
"""
...
def register_fonts():
    ...
    if not font_found:
        print("[警告] 未找到中文字体，PDF 中文将显示异常")
        return False

    try:
        pdfmetrics.registerFont(TTFont("CN_FONT", font_found))
        print(f"[字体加载成功] {font_found}")
        return True
...
# ===================== PDF 页面构建 =====================
def build_cover_page(story, tweets, font_name):
    story.append(Spacer(1, 60 * mm))
    story.append(ColorBar(PAGE_W - 80 * mm, 2 * mm, COLOR_ACCENT))
    story.append(Spacer(1, 8 * mm))

    title_style = ParagraphStyle(
        "CoverTitle", fontName=font_name, fontSize=32,
    ...
    sub_style = ParagraphStyle(
        "CoverSub", fontName=font_name, fontSize=14,
    ...
    info_style = ParagraphStyle(
        "CoverInfo", fontName=font_name, fontSize=11,
    ...
    src_style = ParagraphStyle(
        "CoverSrc", fontName=font_name, fontSize=9,
...
def build_toc_page(story, tweets, font_name):
    toc_title = ParagraphStyle(
        "TOCTitle", fontName=font_name, fontSize=20,
    ...
    toc_style = ParagraphStyle(
        "TOCItem", fontName=font_name, fontSize=10,
...
def build_tweet_pages(story, tweets, font_name):
    text_style = ParagraphStyle(
        "TweetText", fontName=font_name, fontSize=10,
        textColor=COLOR_TEXT, leading=17, spaceAfter=2 * mm,
        alignment=TA_JUSTIFY
    )
    quote_style = ParagraphStyle(
        "QuoteText", fontName=font_name, fontSize=9,
        textColor=COLOR_TEXT_LIGHT, leading=15, spaceAfter=2 * mm,
        leftIndent=8 * mm
    )
    section_title = ParagraphStyle(
        "SectionTitle", fontName=font_name, fontSize=16,
        textColor=COLOR_PRIMARY, leading=24, spaceAfter=3 * mm
    )
    img_caption = ParagraphStyle(
        "ImgCaption", fontName=font_name, fontSize=8,
    ...
    for i, tweet in enumerate(tweets, 1):
        ...
        stats_style = ParagraphStyle(
            "Stats", fontName=font_name, fontSize=8,
        ...
        for j, img_path in enumerate(images, 1):
            try:
                ...
                err_style = ParagraphStyle(
                    "ImgErr", fontName=font_name, fontSize=8,
...
# ===================== 页码 =====================
def add_page_number(canvas, doc, font_name):
    canvas.saveState()
...
def main():
    ...
    story = []
    build_cover_page(story, tweets, font_name)
    build_toc_page(story, tweets, font_name)
    build_tweet_pages(story, tweets, font_name)

    doc.build(
        story,
        onFirstPage=lambda c, d: None,
        onLaterPages=lambda c, d: add_page_number(c, d, font_name)
    )
